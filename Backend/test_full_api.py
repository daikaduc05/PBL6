...
def check_status(response, expected: int, action: str):
    if response.status_code == expected:
        ...
        console.print(f"[red]✗ {action} thất bại. Expected {expected}, got {response.status_code}[/red]")
...
def run_tests():
    ...
    res = client.post("/categories", json={"name": f"Cat 1 {uid}", "slug": f"cat-1-{uid}"}, headers=auth_headers("admin"))
    check_status(res, 201, "Tạo Category 1")
    cat1_id = res.json()["id"]

    step("C2. Tạo Category 2 (Làm con của Cat 1)")
    res = client.post("/categories", json={"name": f"Cat 2 {uid}", "slug": f"cat-2-{uid}", "parent_id": cat1_id}, headers=auth_headers("admin"))
    ...
    res = client.put(f"/categories/{cat1_id}", json={"name": "Cat 1 Updated"}, headers=auth_headers("admin"))
    ...
    res = client.put(f"/lessons/{lesson_id}", json={"title": "Test Lesson Updated"}, headers=auth_headers("admin"))
    ...
    res = client.put(f"/lessons/{lesson_id}", json={"title": "Fail Update"}, headers=auth_headers("editor"))
    ...
    res = client.get(f"/lessons/{vip_lesson_id}", headers={"X-User-Id": "123", "X-User-Roles": "student", "X-User-Tier": "normal"})
    ...
    res = client.get(f"/lessons/{vip_lesson_id}", headers={"X-User-Id": "123", "X-User-Roles": "student", "X-User-Tier": "vip"})
