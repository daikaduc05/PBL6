...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("demo_vip.log", mode="w"), logging.StreamHandler()],
)
...
def run_demo():
    ...
    cat_a = client.post(
        "/categories",
        json={"name": f"Cat A {uid}", "slug": f"cat-a-{uid}"},
        headers=auth_headers("admin"),
    ).json()
    cat_b = client.post(
        "/categories",
        json={"name": f"Cat B {uid}", "slug": f"cat-b-{uid}"},
        headers=auth_headers("admin"),
    ).json()
    ...
    lessons_data = [
        {
            "title": "Lesson 1 (Public)",
            "body": "Nội dung public 1",
            "level": "easy",
            "access": "public",
            "category_id": cat_a["id"],
        },
        {
            "title": "Lesson 2 (Public)",
            "body": "Nội dung public 2",
            "level": "medium",
            "access": "public",
            "category_id": cat_a["id"],
        },
        {
            "title": "Lesson 3 (VIP)",
            "body": "Tuyệt chiêu IELTS (VIP 1)",
            "level": "hard",
            "access": "vip",
            "category_id": cat_a["id"],
        },
        {
            "title": "Lesson 4 (Public)",
            "body": "Nội dung public 4",
            "level": "easy",
            "access": "public",
    ...
    for item in items:
        ...
        if item["id"] in lesson_ids:
            logging.info(
                f"[Normal User] {item['title']} ({item['access'].upper()}) -> Body: '{item['body']}'"
            )
            time.sleep(0.3)
    ...
    for item in items:
        if item["id"] in lesson_ids:
            logging.info(
                f"[VIP User] {item['title']} ({item['access'].upper()}) -> Body: '{item['body']}'"
            )
            time.sleep(0.3)
