...
from content.schemas.category import CategoryCreate, CategoryUpdate, CategoryResponse, CategoryTreeResponse
...
class CategoryService:
    ...
    def _build_tree(self, categories: list[Category]) -> list[CategoryTreeResponse]:
        category_map = {
            cat.id: CategoryTreeResponse.model_validate(cat) 
    ...
    async def create_category(self, data: CategoryCreate) -> Category:
        ...
        if existing_slug:
            raise HTTPException(status_code=400, detail="Slug này đã được sử dụng. Vui lòng chọn slug khác.")
    ...
    async def delete_category(self, category_id: uuid.UUID) -> None:
        ...
        if children:
            raise HTTPException(status_code=400, detail="Không thể xóa vì danh mục này đang chứa các danh mục con.")
